...
#How it accomplishes this: The script multiplies the 10th position by 1, X position by 10, and the 100th position by 100.
...
#The main function is to call the underlying functions: randomSeqToAmino, and to print out all the results.
def main():
...
def randomSeqToAmino(N):
	...
	for index, i in enumerate(xrange(3)):
		...
		if index > 0:
			...
			#concatenates the arrary into the hundreds place
			dnaSeqFrame = np.dot(reshapeArrary, [100, 10, 1])

			#dictionary lookup from the variable named "NestedAminoDic"
			AminoList += [NestedAminoDic[k] for k in dnaSeqFrame]
		else:
			#concatenates the arrary into the hundreds place
			dnaSeqFrame = np.dot(randomInt, [100, 10, 1])
